...
router.get('/callback', async (req, res) => {
    ...
    try {
        ...
        const tokens = await youtube_service_1.youtubeService.exchangeCodeForTokens(code);
 ...
 */
router.get('/stats', auth_middleware_1.requireAuth, async (req, res, next) => {
    const userId = req.user.id;
    try {
        const supabase = (0, supabase_service_1.getSupabaseClient)();
        ...
        // Calculate analytics
        const analytics = youtube_service_1.youtubeService.calculateAnalytics(channelStats, recentVideos);
