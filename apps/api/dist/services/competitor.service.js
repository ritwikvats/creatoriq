...
class CompetitorService {
     ...
     */
    async lookupInstagram(accessToken, igUserId, competitorUsername) {
        try {
            ...
            const response = await axios_1.default.get(`${INSTAGRAM_API_BASE}/${igUserId}`, {
                params: {
     ...
     */
    async lookupYouTube(accessToken, competitorHandle) {
        try {
            // Clean up handle
            ...
            const videoPosts = recentVideos.map((v) => ({
            ...
            return {
                ...
                avgComments,
                postsPerWeek,
                recentPosts: recentVideos.map((v) => ({
    ...
    calculatePostsPerWeek(posts) {
        ...
        if (spanWeeks < 0.1)
            return timestamps.length; // All in same day basically
