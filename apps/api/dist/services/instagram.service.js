...
exports.getBestPostingTimes = exports.getAudienceDemographics = exports.getMediaPerformance = exports.getAccountInsights = exports.getInstagramAccountId = exports.getAccessToken = exports.getAuthUrl = void 0;
...
const getAccessToken = async (code) => {
    ...
    const response = await axios_1.default.get(`${INSTAGRAM_API_BASE}/oauth/access_token`, {
        params: {
...
const getInstagramAccountId = async (accessToken) => {
    try {
        ...
        let businessPages = [];
        try {
            const businessResponse = await axios_1.default.get(`${INSTAGRAM_API_BASE}/me/businesses`, {
                params: {
            ...
            if (businessResponse.data.data && businessResponse.data.data.length > 0) {
                ...
                const businessPagesResponse = await axios_1.default.get(`${INSTAGRAM_API_BASE}/${businessId}/client_pages`, {
                    params: {
                        access_token: accessToken,
        ...
        const pagesResponse = await axios_1.default.get(`${INSTAGRAM_API_BASE}/me/accounts`, {
            params: {
                access_token: accessToken,
                fields: 'id,name,instagram_business_account'
            },
        ...
        console.log(`📄 Found ${pages.length} Facebook Page(s) total:`, pages.map((p) => ({ name: p.name, hasInstagram: !!p.instagram_business_account })));
        if (pages.length === 0) {
...
exports.getInstagramAccountId = getInstagramAccountId;
const getAccountInsights = async (accessToken, accountId) => {
    try {
        console.log(`📊 Fetching insights for Instagram account: ${accountId}`);
        // Get account info with ALL available fields
        const accountResponse = await axios_1.default.get(`${INSTAGRAM_API_BASE}/${accountId}`, {
            params: {
        ...
        try {
            ...
            const insightsResponse = await axios_1.default.get(`${INSTAGRAM_API_BASE}/${accountId}/insights`, {
                params: {
...
const getMediaPerformance = async (accessToken, accountId, limit = 10) => {
    // Get recent media with ALL available fields
    const mediaResponse = await axios_1.default.get(`${INSTAGRAM_API_BASE}/${accountId}/media`, {
        params: {
    ...
    const mediaWithInsights = await Promise.all(media.map(async (item) => {
        try {
            const insightsResponse = await axios_1.default.get(`${INSTAGRAM_API_BASE}/${item.id}/insights`, {
                params: {
...
exports.getMediaPerformance = getMediaPerformance;
/**
 ...
 */
const getAudienceDemographics = async (accessToken, igAccountId) => {
    try {
        const demographics = {
            cities: [],
            countries: [],
        ...
        try {
            ...
            const countryResponse = await axios_1.default.get(`${INSTAGRAM_API_BASE}/${igAccountId}/insights`, {
                params: {
        ...
        try {
            ...
            const cityResponse = await axios_1.default.get(`${INSTAGRAM_API_BASE}/${igAccountId}/insights`, {
                params: {
                    metric: 'follower_demographics',
                    period: 'lifetime',
                    breakdown: 'city',
        ...
        try {
            ...
            const ageGenderResponse = await axios_1.default.get(`${INSTAGRAM_API_BASE}/${igAccountId}/insights`, {
                params: {
                    metric: 'follower_demographics',
                    period: 'lifetime',
...
exports.getAudienceDemographics = getAudienceDemographics;
/**
 * Get best posting times based on follower activity
 */
const getBestPostingTimes = async (accessToken, igAccountId) => {
    try {
        // Fetch online followers insights
        const response = await axios_1.default.get(`${INSTAGRAM_API_BASE}/${igAccountId}/insights`, {
            params: {
        ...
        const mediaResponse = await axios_1.default.get(`${INSTAGRAM_API_BASE}/${igAccountId}/media`, {
            params: {
...
exports.getBestPostingTimes = getBestPostingTimes;
