...
export const aiService = {
    ...
    async generateAnalyticsReport(analytics: any): Promise<any> {
        ...
        if (openClawService.isAvailable()) {
            try {
                apiLogger.info('Using OpenClaw for analytics report');
                return await openClawService.generateAnalyticsReport(analytics);
            } catch (error) {
                apiLogger.warn('OpenClaw failed for analytics report, falling back to Groq', { error });
            }
        }

        // Fallback to Groq
        try {
            apiLogger.info('Using Groq (fallback) for analytics report');
            const groq = getGroqClient();
    ...
    async generateCompetitorAnalysis(platform: string, userStats: any, competitorStats: any): Promise<any> {
        ...
        if (openClawService.isAvailable()) {
            try {
                apiLogger.info('Using OpenClaw for competitor analysis');
                return await openClawService.generateCompetitorAnalysis(platform, userStats, competitorStats);
            } catch (error) {
                apiLogger.warn('OpenClaw failed for competitor analysis, falling back to Groq', { error });
            }
        }

        // Fallback to Groq
        try {
            apiLogger.info('Using Groq (fallback) for competitor analysis');
            const groq = getGroqClient();
