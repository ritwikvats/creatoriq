 ...
 */
export async function getLatestTaxUpdates(): Promise<string> {
 ...
 */
export function calculateTaxLiability(annualIncome: number): {
    gstRequired: boolean;
    estimatedGST: number;
    ...
    const gstRequired = annualIncome >= CURRENT_TAX_RULES.gst.threshold;
    ...
    for (const slab of CURRENT_TAX_RULES.incomeTax.slabs2026) {
...
export async function getTaxAdvice(params: {
    annualIncome: number;
    revenueBreakdown: { youtube?: number; instagram?: number; brandDeals?: number };
    expenses?: number;
}): Promise<string> {
...
- GST Threshold: ₹${CURRENT_TAX_RULES.gst.threshold.toLocaleString('en-IN')}
- TDS Rate: ${CURRENT_TAX_RULES.tds.rates.brandDeals}%
    ...
    try {
        ...
        const completion = await groq.chat.completions.create({
            messages: [
                {
                    role: 'system',
                    content: `You are an expert Indian CA. Current date: ${new Date().toISOString().split('T')[0]}. Financial Year: ${CURRENT_TAX_RULES.financialYear}.`,
                },
 ...
 */
export async function syncTaxRules(): Promise<void> {
